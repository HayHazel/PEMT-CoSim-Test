...
def readtmy3(filename=None, coerce_year=None, recolumn=True):
    ...
    data = pd.read_csv(
        ...
        date_parser=lambda x: _parsedate(x, year=coerce_year), index_col=0)
    if recolumn:
...
def _parsedate(date_str, year=None):
    try:
        time = datetime.strptime(date_str, "%m/%d/%Y %H:%M")
    except ValueError:
        time = datetime.strptime(date_str.replace('24:', '23:'), "%m/%d/%Y %H:%M")
        time += timedelta(hours=1)
    if year is not None:
        true_date = time.replace(year=year)
    return time
...
def weathercsv(tmyfile, outputfile, start_time, end_time, year_):
    ...
    dts.to_csv('./fed_weather/' + outputfile, float_format='%.4f')
...
def _tests():
    ...
    weathercsv('../../support/weather/TX-Houston_Bush_Intercontinental.tmy3', 'weather.csv', '2000-01-01 00:00:00',
