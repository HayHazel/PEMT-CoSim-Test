...
try:
    import my_tesp_support_api.fncs as fncs
except:
    pass
try:
    import helics
...
def summarize_opf(res):
    ...
    idx = 1
    for row in gen:
        print('{:4d} {:4d} {:8.2f} {:8.2f} {:9.5f} {:9.5f} {:9.5f} {:9.5f}'.format(idx, int(row[0]), float(row[1]),
                                                                                   float(row[2]), float(row[21]),
...
def pypower_loop(casefile, rootname, helicsConfig=None):
    ...
    if helicsConfig is not None:
        print("HELICS configuration", flush=True)
        ...
        for i in range(subCount):
            ...
            if 'distribution_load' in target:
                sub_load = sub
        print("Initialising", sub_c2, sub_c1, sub_deg, sub_max, sub_unresp, sub_load, flush=True)
        helics.helicsFederateEnterInitializingMode(hFed)
    ...
    while ts <= tmax:
        ...
        if hFed is not None:  # HELICS inputs
            ...
            if (sub_max is not None) and helics.helicsInputIsUpdated(sub_max):
                ...
                resp_max = helics.helicsInputGetComplex(
                    sub_max).real * load_scale  # TODO: pyhelics needs to return complex instead of tuple
            ...
            if (sub_load is not None) and helics.helicsInputIsUpdated(sub_load):
                gld_load = helics.helicsInputGetComplex(
                    sub_load)  # TODO: pyhelics needs to return complex instead of tuple
        ...
        if ts >= tnext_metrics:
            ...
            for i in range(dsoBus.shape[0]):
                ...
                bus_metrics[str(ts)][str(busnum)] = [met[0] / n_accum, met[1] / n_accum, met[2] / n_accum,
                                                     met[3] / n_accum,
