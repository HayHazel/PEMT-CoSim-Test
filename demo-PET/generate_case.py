...
class GLOBAL_Configuration:
    ...
    ratio_PV_only_list = [10/10] #[0/10] # ratio of houses that have only PV installed for each VPP
    ...
    ratio_PV_Bat_list =  [0/30] #[30/30] # ratio oh houses that have both PV and battery installed for each VP
...
weathercsv (tmy_file, 'weather.dat', glbal_config.start_time, glbal_config.stop_time, glbal_config.year) # it will output weather.dat in the weather fold as the input of the weather federate
