...
import subprocess
import pickle
from PET_Prosumer2 import HOUSE, VPP        # import user-defined my_hvac class for hvac controllers
...
hour_stop = 24 # simulation duration (default 48 hours)
...
if drawFigure:
  fig, (ax1, ax2, ax3, ax4, ax5) = plt.subplots(5) #previously subplots(5) and added ax6 to left side
...
while (time_granted < StopTime):
  ...
  if drawFigure and time_granted >= tnext_fig_update:
    ...
    #my own attempt at added bid pricings 
    #ax6.cla()
    #ax6.set_ylabel("Price ($)")
    #ax6.plot(curves.time_hour_curve, curves.curve_price)
