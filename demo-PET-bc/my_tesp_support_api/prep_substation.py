...
def ProcessGLM (fileroot, global_config):
    ...
    meta = {'markets':auctions, 'VPPs':glm_dict['VPPs'],
            ...
            'hvacs':controllers, 'inverters': glm_dict['inverters'], 'dt':dt, 'duration': duration, 'GridLABD':FedName}
    ...
    for key, val in glm_dict['inverters'].items():
        ...
        if val['resource'] == 'battery':
            ...
            subs.append ({"key":"gld1/"+resource_name+"#battery_state", "type":"string"})
...
#        print (days, seconds)
    ...
    for key, val in glm_dict['inverters'].items():
        ...
        if val['resource'] == 'battery':
            ...
            prop = 'battery_state'
            pubs.append ({"global":False, "key":resource_name + '#' + prop, "type":"string", "info":{"object":resource_name,"property":prop}})
    ...
    for key, val in inverts_dict.items():
        if val['resource'] == 'battery':
            pass
        elif val['resource'] == 'solar':
            pass
    ...
    # write pypower system configuration file
    with open('./fed_pypower/te30_pp.json', encoding='utf-8') as f:
        pypower_config = json.loads(f.read()) # federate_config is the dict data structure
        f.close()
    pypower_config['StartTime'] = global_config.start_time
    pypower_config['Tmax'] = global_config.duration
    unitout_period = 48*3600
    unitout_duration = 13*3600
    number_gens = len(pypower_config['gen'])
    UnitsOut = []

    for t in range(108000, duration, unitout_period):
        UnitsOut.append([2, t, t+unitout_duration])

    pypower_config['UnitsOut'] = UnitsOut

    wp = open ('./fed_pypower/te30_pp.json', 'w')
    json.dump (pypower_config, wp, ensure_ascii=False, indent=2)
    wp.close()
