...
class FEDERATE_HELPER:
    def __init__(self, configfile, helicsConfig, metrics_root, hour_stop):
        ...
        for i, houseName in enumerate(self.house_name_list):
            ...
            self.housesInfo_dict[houseName]['battery'] = None # battery used to regulate the output of HVAC
            self.housesInfo_dict[houseName]['battery2'] = None # battery used to generate and consume the power packet
        ...
        for key, dict in self.agents_dict['inverters'].items():
            ...
            if billingmeter_id in self.billingmeter_name_list:
                ...
                if resource == 'battery' and 'batt_' in key:
                    self.housesInfo_dict[house_name]['battery'] = key
                if resource == 'battery' and 'batt2_' in key:
                    self.housesInfo_dict[house_name]['battery2'] = key
        ...
        self.subsBattState = {} # subscriptions dict for battery state
        self.subsBatt2Power = {}  # subscriptions dict for battery2 measured_power
        self.subsBatt2SoC = {}  # subscriptions dict for battery2 SoC
        self.subsBatt2State = {} # subscriptions dict for battery state
    ...
    def register_pubssubs(self):
        ...
        for house_name, val in self.housesInfo_dict.items():
            ...
            if val['battery'] != None:
                ...
                self.subsBattState[house_name] =  helics.helicsFederateGetSubscription (self.hFed, battSubTopic + '#battery_state')
            ...
            if val['battery2'] != None:
                battery_meter_name = self.agents_dict['inverters'][val['battery2']]['parent']
                battery_name = self.agents_dict['inverters'][val['battery2']]['resource_name']
                battMeterSubTopic = self.gldName + '/' + battery_meter_name
                battSubTopic = self.gldName + '/' + battery_name
                self.subsBatt2Power[house_name] =  helics.helicsFederateGetSubscription (self.hFed, battMeterSubTopic + '#measured_power')
                self.subsBatt2SoC[house_name] =  helics.helicsFederateGetSubscription (self.hFed, battSubTopic + '#state_of_charge')
                self.subsBatt2State[house_name] =  helics.helicsFederateGetSubscription (self.hFed, battSubTopic + '#battery_state')

                battery_inv_name = val['battery2']
                battCtlPubTopic = self.fedName + '/' + battery_inv_name
                self.pubsBatt2Pout[house_name] = helics.helicsFederateGetPublication (self.hFed, battCtlPubTopic + '/P_Out')
                self.pubsBatt2Qout[house_name] = helics.helicsFederateGetPublication (self.hFed, battCtlPubTopic + '/Q_Out')
    ...
    def get_agent_pubssubs(self,key, category, info = None):
        ...
        if category == 'house': # the key is the house name
            ...
            if info['battery'] != None:
                ...
                agent_subs['subBattState'] = self.subsBattState[key]
            ...
            if info['battery2'] != None:
                agent_subs['subBatt2Power'] = self.subsBatt2Power[key]
                agent_subs['subBatt2SoC'] = self.subsBatt2SoC[key]
                agent_subs['subBatt2State'] = self.subsBatt2State[key]
                agent_pubs['pubBatt2Pout'] = self.pubsBatt2Pout[key]
                agent_pubs['pubBatt2Qout'] = self.pubsBatt2Qout[key]
...
class CURVES_TO_PLOT:
    def __init__(self, num_prosumers):
        ...
        self.house_kwh = []
        self.curve_house_kwh = deque(maxlen = 300)
        ...
        self.system_PV = []
        self.curve_system_PV = deque(maxlen = number_points)
        self.house_PV_mean = []
        self.house_PV_max = []
        self.house_PV_min = []
        self.system_Batt = []
        self.curve_system_Batt = deque(maxlen = number_points)
        self.house_battpower_mean = []
        self.house_battpower_max = []
        self.house_battpower_min = []
        self.house_SoC_mean = []
        self.curve_house_SoC_mean = deque(maxlen = number_points)
        self.house_SoC_max = []
        self.house_SoC_min = []
        ...
        # about auction
        self.LMP = []
        self.curve_LMP = deque(maxlen = number_points)

        self.cleared_price = []
        self.curve_cleared_price = deque(maxlen = number_points)

        self.cleared_quantity = []
        self.curve_cleared_quantity = deque(maxlen = number_points)
        ...
        self.curve_consumerSurplus = deque(maxlen = number_points)

        self.supplierSurplus = []
        self.curve_supplierSurplus = deque(maxlen = number_points)

        self.social_welfare_combined_surplus = []
        self.curve_social_welfare_combined_surplus = deque(maxlen = number_points)

        self.social_welfare_seller_income = []
        self.curve_social_welfare_seller_income = deque(maxlen = number_points)

        self.time_secend_curve = deque(maxlen =10000)
    ...
    def save_statistics(self, path):
        ...
        data_dict['cleared_quantity'] = self.cleared_quantity
        data_dict['LMP'] = self.LMP
        data_dict['social_welfare_combined_surplus'] = self.social_welfare_combined_surplus
        data_dict['social_welfare_seller_income'] = self.social_welfare_seller_income
        ...
        data_dict['system_Batt'] = self.system_Batt
        ...
        data_dict['house_SoC_mean'] = self.house_SoC_mean
        data_dict['house_SoC_max'] = self.house_SoC_max
        data_dict['house_SoC_min'] = self.house_SoC_min
    ...
    def record_auction_statistics(self, seconds, houseObjs, aucObj):
        ...
        self.buyer_ratio.append(aucObj.public_info['ratio-buyer'])
        self.seller_ratio.append(aucObj.public_info['ratio-seller'])
        self.nontcp_ratio.append(aucObj.public_info['ratio-nontcp'])

        self.LMP.append(aucObj.lmp)
        self.cleared_price.append(aucObj.clearing_price)
        self.cleared_quantity.append(aucObj.clearing_quantity)
        self.consumerSurplus.append(aucObj.consumerSurplus)
        self.supplierSurplus.append(aucObj.supplierSurplus)
        self.social_welfare_combined_surplus.append(aucObj.social_welfare_combined_surplus)
        self.social_welfare_seller_income.append(aucObj.social_welfare_seller_income)


    def record_house_kwh(self, house):
        self.house_kwh.append(house.house_kwh)
    ...
    def record_state_statistics(self, seconds, houseObjs, aucObj, vpp):
        ...
        for key, house in houseObjs.items():
            ...
            battery_power_list.append(house.battery2_kw)
            battery_soc_list.append(house.battery2_SoC)
        ...
        self.system_Batt.append(sum(battery_power_list))
        ...
        self.house_battpower_mean.append(sum(battery_power_list)/len(battery_power_list))
        self.house_battpower_max.append(max(battery_power_list))
        self.house_battpower_min.append(min(battery_power_list))
        self.house_SoC_mean.append(sum(battery_soc_list)/len(battery_soc_list))
        self.house_SoC_max.append(max(battery_soc_list))
        self.house_SoC_min.append(min(battery_soc_list))
    ...
    def update_curves_house_kwh(self, seconds):
        self.time_secend_curve.append(seconds)
        self.curve_house_kwh.append(self.house_kwh[-1])
    ...
    def update_curves(self, seconds):
        ...
        self.curve_system_PV.append(self.system_PV[-1])
        self.curve_system_Batt.append(self.system_Batt[-1])

        self.curve_hvac_on_ratio.append(self.hvac_on_ratio[-1])
        self.curve_house_SoC_mean.append(self.house_SoC_mean[-1])
        ...
        self.curve_LMP.append(self.LMP[-1])
        self.curve_cleared_quantity.append(self.cleared_quantity[-1])
        self.curve_social_welfare_combined_surplus.append(self.social_welfare_combined_surplus[-1])
        self.curve_social_welfare_seller_income.append(self.social_welfare_seller_income[-1])
