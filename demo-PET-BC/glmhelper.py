...
class GLM_HELPER:
    ...
    def __init__(self, config):
        ...
        with open('./fed_gridlabd/glm-template/battery_template', 'r') as f:
            batt_code = f.read()
        global batt2_code
        with open('./fed_gridlabd/glm-template/battery2_template', 'r') as f:
            batt2_code = f.read()
    ...
    def configure_clock(self):
        """
        configure the time clock for the .glm file
        """
        replaceInPattern(self.file_name, "{starttime}", self.start_time)
        replaceInPattern(self.file_name, "{stoptime}", self.stop_time)
    ...
    def configure_houses(self):
        ...
        for vpp_idx in range(self.num_VPPs):
            ...
            for house_idx in range(num_houses_phase):
                ...
                h_code += self.configure_battery(h_par_dict, vpp_idx, phase, house_idx) # this battery is used for HVAC, every house should have
                ...
                if count_pv_only>0:
                    ...
                    h_code += self.configure_battery2(h_par_dict, vpp_idx, phase, house_idx)
                    count_bat_only -= 1
    ...
    def configure_battery2(self, house_par_dict, vpp_idx, phase, house_idx):
        ...
        battery_capacity = 10 # 10 kWh
        random.seed(seed)
        state_of_charge = round(random.uniform(0.01,0.99),2)
